...
class AyahByAyahDataSourceImpl implements AyahByAyahLocalDataSource {
  ...
  Future<List<AyahByAyahModel>> searchAyahs({required String query, required String translationColumn}) async {
    ...
    if (isArabic) {
      final result = await _database.rawQuery(
        ...
        FROM ${TableNames.tableOfAyahFts} fts
        ...
        WHERE ${TableNames.tableOfAyahFts} MATCH ?
    ...
    final result = await _database.rawQuery(
      ...
      SELECT
        ...
        a.${ColumnNames.ayahPosition},
        COALESCE(t.$translationColumn, '') AS ${ColumnNames.ayahTranslation}
      FROM ${TableNames.tableOfTranslationFts} fts
      JOIN ${TableNames.tableOfAyah} a ON a.${ColumnNames.ayahId} = fts.rowid
      LEFT JOIN ${TableNames.tableOfTranslations} t ON t.id = a.${ColumnNames.ayahId}
      WHERE ${TableNames.tableOfTranslationFts} MATCH ?
      ORDER BY a.${ColumnNames.surahNumber}, a.${ColumnNames.ayahNumber}
      ''',
  ...
  @override
  Future<List<AyahByAyahModel>> fetchAyahsByIds({required List<int> ayahIds, required String translationColumn}) async {
    ...
    models.sort((a, b) => (orderMap[a.ayahId] ?? 1 << 30).compareTo(orderMap[b.ayahId] ?? 1 << 30));
